...
export class ScheduleAuditModalComponent {
  ...
  onDocumentClick(event: Event): void {
    ...
    const clickedInsideSupplier = this.supplierDropdownRef?.nativeElement?.contains(target);
    const clickedInsideAuditor = this.auditorDropdownRef?.nativeElement?.contains(target);
    const clickedInsideAuditType = this.auditTypeDropdownRef?.nativeElement?.contains(target);

    if (!clickedInsideSupplier && !clickedInsideAuditor && !clickedInsideAuditType) {
